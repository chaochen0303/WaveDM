...
# There are two approaches of units, we use the one presented in PyUltralight
...
# use this function in the Jupyter Notebook
def convert(value, unit, type, lamb):
    converted = 0
    if (type == 'l'):
        if (unit == ''): #just rescale units
    ...
    elif (type == 'N'): # total number of particles
        if (unit == ''):
            converted = value*lamb
        elif (unit == 'normal'):
            converted = value*lamb/1e+96 * 1.51 * (axion_mass/1e-23/ 1.783e-36)**(5/2)
        else: 
            raise NameError('Unsupported speed unit used')
            
    elif (type == 'density'): # energy density
        if (unit == ''):
            converted = value*lamb**4
        elif (unit == 'Msun_kpc-3'):
            converted = value*lamb**4/3.2
        else: 
            raise NameError('Unsupported speed unit used')
...
#------creat grids in x and k spaces (turn on sparse)------------------
...
def grid_func_3dplot(resol, length):
    #resol: the number of grids on each side in the real space
    #length: the normalized length of each side in the real space
    ncell = resol**3   # the number of cells     
    vcell = length**3 / ncell    # the normalized volume of each cell
    ...
    grid_vec = np.linspace( - length/2.0 + length/ float(2.0*resol), 
                       length/2.0 - length/float(2.0*resol), resol) # (-length/2, length/2) on each sides
    xarray, yarray, zarray = np.meshgrid(grid_vec, grid_vec, grid_vec, sparse=False, indexing ='ij') #(resol, resol,resol) for x
...
def soliton_formaiton(resol, length, Ntot,ini_psi_plot, 
    ...
    if (ini_psi_plot):
        ...
        plt.imshow((numdensity[:resol**2]).reshape((resol,resol)), cmap="jet",
           interpolation="nearest", origin="lower", norm=matplotlib.colors.LogNorm(1e-2, 1e+2),
          extent=[-length/2.0, length/2.0, -length/2.0, length/2.0])
        plt.colorbar(pad=0.1)
        plt.xlabel("x")
        plt.ylabel("y")
